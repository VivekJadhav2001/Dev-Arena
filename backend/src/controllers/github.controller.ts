...
function publicStats(user: {
  ...
  return {
    ...
    lastSyncedAt: user.githubStats.lastSyncedAt,
  };
}
...
export async function syncGitHub(
  ...
  try {
    ...
    if (lastSyncedAt && Date.now() - lastSyncedAt.getTime() < CACHE_MS)
      return res.success(
        200,
        "GitHub statistics are up to date",
        publicStats(user),
      );
    const syncedUser = await syncGitHubUser(user.id);
    return res.success(200, "GitHub statistics synced", publicStats(syncedUser));
  } catch (error) {
    next(error);
  }
}

export async function getMyGitHubStats(
  req: Request,
  res: Response,
  next: NextFunction,
) {
